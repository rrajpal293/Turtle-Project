...
class SimResults():  
    ...
    def Append(self, dt, Signal, EntryLevel, ExitLevel, Units):
    	if sum(_data['Date'] == dt) == 0: # if date does not exist then append otherwise update
    		_data.append({'data' : dt, 'Signal': Signal, 'Entry Level' : EntryLevel, 'Exit Level' : ExitLevel, 'BOD Units' : BODUnits, 'EODUnits' : EODUnits})
    	else:
    ...
    def Update(self, dt, strField, value):
        if sum(_data['Date'] == dt) == 1: # only update if date already exists, otherwise append
